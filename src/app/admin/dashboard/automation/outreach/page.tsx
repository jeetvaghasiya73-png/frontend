...
> {
  ...
  componentDidCatch(error: Error, errorInfo: any) {
    console.error("OutreachManager ErrorBoundary caught an exception:", error, errorInfo);
  }
      ...
      return (
        <div className="p-8 max-w-xl mx-auto my-12 bg-white dark:bg-[#111111] border border-red-500/20 rounded-2xl text-left space-y-4 shadow-xl">
          <div className="flex items-center gap-3 text-red-500">
            ...
            <h2 className="text-lg font-bold font-mono">Outreach Console Recovery Mode</h2>
          </div>
          <p className="text-xs text-gray-600 dark:text-gray-400 font-mono leading-relaxed">
            {this.state.error?.message || "An error occurred while rendering the outreach component."}
          </p>
          ...
          {lines.map((line, lineIdx) => {
            ...
            return (
              <li key={lineIdx} className="leading-relaxed flex items-start gap-2">
                <span className="mt-1.5 shrink-0 text-indigo-600 dark:text-indigo-400 select-none text-[8px]">•</span>
...
function OutreachManager() {
  ...
  // Autoscroll outreach terminal (instant scroll to prevent browser main thread locks)
  useEffect(() => {
  ...
  const updateActivityThrottled = (data: any) => {
    ...
    if (!updateTimeoutRef.current) {
      ...
      }, 300); // Batch log/status renders to run smoothly at ~3 FPS
    }
  };
  ...
  // 2. Refetch when window regains focus (no periodic HTTP polling loops)
  useEffect(() => {
  ...
  // 3. Set up WebSocket for real-time live activity stream (pure WebSocket push, no HTTP polling)
  useEffect(() => {
    if (!isAuthenticated) return;
    ...
    <div className="space-y-6 text-left pb-16 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => fetchData()}
            className="p-1.5 hover:bg-gray-150 dark:hover:bg-white/5 border border-gray-250 dark:border-white/10 rounded-lg text-gray-450 hover:text-gray-900 dark:hover:text-white transition-colors cursor-pointer flex items-center gap-1 text-xs font-mono"
            title="Refresh Data"
          >
      ...
      <div className="mt-4">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="border border-gray-150 dark:border-white/5 bg-gray-50/10 dark:bg-white/1 rounded-xl p-5 space-y-5">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 sm:gap-2 relative">
                    ...
                    <div className="flex flex-col items-center text-center z-10">
                      ...
                      <span className="text-[9px] text-gray-400 font-mono mt-0.5">{analytics?.total_leads ?? 0} Leads</span>
                    </div>

                    {/* Step 2: Queue */}
                    <div className="flex flex-col items-center text-center z-10">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center border transition-all duration-300 ${
                        (analytics?.total_queued ?? 0) > 0 
                    ...
                    <div className="flex flex-col items-center text-center z-10">
                      ...
                      <span className="text-[9px] text-gray-400 font-mono mt-0.5">{analytics?.total_sent ?? 0} Sent</span>
                    </div>

                    {/* Step 5: Classification */}
                    <div className="flex flex-col items-center text-center z-10">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center border transition-all duration-300 ${
                        (analytics?.total_replied ?? 0) > 0 
                      ...
                      <span className="text-[9px] text-gray-400 font-mono mt-0.5">{analytics?.total_replied ?? 0} Classified</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-gray-150 dark:border-white/5 bg-gray-50/10 dark:bg-white/1 rounded-xl p-5 space-y-3">
                  <div className="flex justify-between items-center text-xs font-mono font-bold">
                    ...
                    <span className="text-accent-custom">{analytics?.total_sent ?? 0} / {analytics?.total_leads ?? 0} Sent ({analytics && analytics.total_leads > 0 ? Math.round(((analytics.total_sent || 0) / analytics.total_leads) * 100) : 0}%)</span>
                  </div>
                  <div className="w-full bg-gray-250 dark:bg-white/5 h-2 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${analytics && analytics.total_leads > 0 ? Math.min(100, (((analytics.total_sent || 0) / analytics.total_leads) * 100)) : 0}%` }}
                    />
                  </div>
                </div>
        ...
        {activeTab === "analytics" && (
          analytics ? (
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-[#0c0c0c] border border-gray-200 dark:border-white/10 p-5 rounded-2xl">
                  ...
                  <div className="w-full h-[220px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={analytics.sent_by_day && Array.isArray(analytics.sent_by_day) && analytics.sent_by_day.length > 0 ? analytics.sent_by_day : [{ day: "No Data", count: 0 }]} barSize={26}>
                ...
                <div className="bg-white dark:bg-[#0c0c0c] border border-gray-200 dark:border-white/10 p-5 rounded-2xl">
                  ...
                  <div className="w-full h-[220px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={analytics.category_performance && Array.isArray(analytics.category_performance) && analytics.category_performance.length > 0 ? analytics.category_performance : [{ category: "No Data", sent: 0, replied: 0 }]} barSize={16}>
                        <CartesianGrid vertical={false} strokeDasharray="3 3" stroke={isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.06)"} />
            ...
            <div className="p-12 text-center text-gray-400 font-mono text-xs border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-[#0a0a0a]">
              Analytics data is currently loading or unavailable.
            </div>
          )
        )}
      </div>
